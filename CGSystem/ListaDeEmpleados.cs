    ...
    public partial class ListaDeEmpleados : Form
    {
        ...
        public void Buscar()
        {
            try
            {
                ...
                {
                    string consulta = "SELECT emp.numero_empleado Numero, emp.nombre Nombre, emp.apellido Apellido, emp.cedula_empleado Cedula, pos.descripcion_posicion Cargo, dep.descripcion_departamento Departamento, emp.sueldo Salario, emp.fecha_ingreso Fecha_Ingreso, emp.fecha_nacimiento Nacimiento, emp.celular Celular, emp.direccion Dirección, sec.descripcion_sector Sector, ciu.descripcion_ciudad Ciudad, emp.telefono_casa Tel_Casa, emp.sexo Genero, est.descripcion_estado Estado FROM empleado emp INNER JOIN posicion pos ON pos.codigo_posicion = emp.codigo_posicion INNER JOIN departamento dep ON dep.codigo_departamento = emp.codigo_departamento INNER JOIN estado est ON est.codigo_estado = emp.codigo_estado INNER JOIN sector sec ON sec.codigo_sector = emp.codigo_sector INNER JOIN ciudad ciu ON ciu.codigo_ciudad = emp.codigo_ciudad WHERE emp.nombre LIKE '%" + tbbuscar.Text + "%'";
